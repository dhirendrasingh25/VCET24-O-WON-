...
router.post("/", async (req, res) => {
    try {
        ...
        const prompt = `User was born on ${dob}, occupation: ${occupation}, ailments: ${ailments}, dependents: ${dependents} (${dependantDescription}), goal: ${goal},monthly income: ${monthly_income} ,mandatory expenses: ${mandatoryExpenses}, current investments: ${JSON.stringify(currentInvestments)}, expected investment duration: ${duration}, loans: ${loans}, and EMI: ${emi}. Please provide a list of recommended investment plans in the following JSON format: { "investmentPlans": [ { "planName": "string", "description": "string", "allocation": { "Equity Funds": number, "Debt Funds": number }, "recommendedFunds": [ { "fundName": "string", "fundType": "string" } ], "monthlyInvestment": number } ] }. The output must be strictly JSON, with no additional text, backticks, or explanations outside the JSON structure.`;
