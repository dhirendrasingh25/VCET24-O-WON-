...
async function calculateExpenses(email_id, startDate, endDate) {
    try {
        console.log(
            `Calculating expenses for ${email_id} from ${startDate} to ${endDate}`,
        );
        ...
        const totalExpenses = user.transactions.reduce(
    ...
    try {
        ...
        if (!["weekly", "monthly", "yearly"].includes(period)) {
            return res.status(400).json({
                error: `Invalid period: ${period}. Must be 'weekly', 'monthly', or 'yearly'.`,
            });
        }
        ...
        console.log(
            `Calculating ${period} expenses for ${email_id} from ${formatDate(startDate)} to ${formatDate(endDate)}`,
        );

        const totalExpenses = await calculateExpenses(
            email_id,
            startDate,
            endDate,
        );
