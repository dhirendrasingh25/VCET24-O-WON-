  ...
  return (
    <section className="py-12 bg-gradient-to-r ">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl font-bold text-center mb-8 text-gray-800">Top Finance Courses in India</h2>
        <motion.div 
            ...
            >
              <Card className="h-full flex flex-col">
                <CardHeader className="p-0 relative">
                  ...
                  <Badge className="absolute top-2 right-2 bg-black">{course.badge}</Badge>
                </CardHeader>
                <CardContent className="flex-grow p-4">
                  <CardTitle className="text-lg font-semibold mb-2">{course.title}</CardTitle>
                  <div className="flex items-center mb-2">
                    <Avatar className="h-6 w-6 mr-2">
                      <AvatarImage src={`https://api.dicebear.com/6.x/initials/svg?seed=${course.author}`} />
                      <AvatarFallback>{course.author[0]}</AvatarFallback>
                    </Avatar>
                    <span className="text-sm text-gray-600">{course.author}</span>
                  </div>
                  <p className="text-xl font-bold text-black">{course.price}</p>
                </CardContent>
      ...
      >
        <h3 className="text-2xl font-semibold mb-4 text-center text-gray-800">Why Choose Our Courses?</h3>
