...
function PlaidAuth({ publicToken }: { publicToken: string }) {
  const [account, setAccount] = useState<{ account: string; routing: string } | null>(null)
...
export default function PlaidIntegration() {
  ...
  useEffect(() => {
    async function fetchLinkToken() {
      try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/create_link_token`)
